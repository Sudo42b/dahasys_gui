...
- [ ] **STRUCT-01**: Create `minias/` package directory with `__init__.py` and `__main__.py`
- [ ] **STRUCT-02**: Extract 5 dataclasses (TestResult, AxisResult, CodeInfo, SetupInfo, LimitInfo) into `minias/models.py`
...
- [ ] **DEDUP-01**: Consolidate 4 duplicated `safe_get()` closures into a single utility function
- [ ] **DEDUP-02**: Deduplicate stop-and-save logic between `_on_stop()` and `_stop_and_save_current()`
- [ ] **DEDUP-03**: Consolidate duplicated fallback port list and code info fallback patterns
...
- [ ] **QUAL-02**: Move inline imports (`time`, `re`) to module top-level where they belong
